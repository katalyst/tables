...
RSpec.describe Katalyst::Tables::Cells::LinkComponent do
  let(:table) { Katalyst::TableComponent.new(collection:) }
  let(:collection) { create_list(:resource, 1) }
  ...
  it "renders column data" do
    expect(data).to match_html(<<~HTML)
      <td class="type-link"><a href="/resources/#{collection.first.id}">#{collection.first.name}</a></td>
    HTML
  end
  ...
  context "with html_options" do
    ...
    it "renders data with html_options" do
      expect(data).to match_html(<<~HTML)
        <td id="ID" class="type-link CLASS" style="style" data-foo="bar" aria-label="LABEL"><a href="/resources/#{collection.first.id}">#{collection.first.name}</a></td>
      HTML
    end
  end
  ...
  context "with nil data value" do
    let(:rendered) { render_inline(table) { |row| row.link(:index) } }
    let(:collection) { create_list(:resource, 1, index: nil) }

    it "renders data as url" do
      expect(data).to match_html(<<~HTML)
        <td class="type-link"><a href="/resources/#{collection.first.id}">/resources/#{collection.first.id}</a></td>
      HTML
    end
  end
  ...
  context "when given a block" do
    ...
    it "renders the custom data" do
      expect(data).to match_html(<<~HTML)
        <td class="type-link"><span><a href="/resources/#{collection.first.id}">#{collection.first.name}</a></span></td>
      HTML
    end
  end
end
